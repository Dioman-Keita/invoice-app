...
export const invoiceSchema = z.object({
    ...
    supplier_name: z
        ...
        .regex(
            supplierNameRegex,
            "Nom invalide : seuls les lettres accentuées, chiffres, espaces, tirets, apostrophes et & sont autorisés"
        ),
    supplier_account_number: z
        .string()
        .min(1, "Le numéro de compte est requis")
        .transform((v) => normalizeAccountNumber(String(v ?? "")))
        .refine((v) => isValidAccountNumber(v), {
            message:
                "Numéro de compte invalide. Doit contenir 6–34 caractères alphanumériques."
        })
        .transform((v) => formatAccountCanonical(v)),
    supplier_phone: z
        .string()
        .min(1, "Le numéro de téléphone est requis")
        .transform((v) => (v ?? "").trim())
        .refine((v) => {
            if (v === '+223' || v === '+223 ') return false;
            const phoneRegex = /^\+223(\s\d{2}){4}$/;
            return phoneRegex.test(v);
        }, {
            message: "Numéro invalide. Format requis : +223 XX XX XX XX"
        }),
    invoice_object: z.string().min(1, "L'objet est requis").max(100, "Maximum 100 caractères"),
    invoice_nature: z.enum(["Paiement", "Acompte", "Avoir"], { error: "Type de facture invalide" }),
    invoice_type: z.enum(["Ordinaire", "Transporteur", "Transitaire"], { error: "Nature de la facture obligatoire" }),
    invoice_status: z.enum(["Oui", "Non"], { error: "Etat de la facture requis" }).default('Non'),

...
function ValidateDateInput({ label, placeholder, name, type="text"}) {
    ...
    return (
        <div className="w-full md:w-[48%] mb-4">
            ...
            <input type={type} placeholder={placeholder} {...register(name)} name={name} id={name} onInput={handleInput} onBlur={handleBlur} className={`w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none ${errors[name] ? "focus:ring-red-500 focus:border-red-700 border-red-500" : "focus:ring-blue-500 focus:border-blue-500"}`} />
            {errors[name] && (

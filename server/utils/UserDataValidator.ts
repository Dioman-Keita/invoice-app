...
class UserDataValidator {
  // Départements autorisés par rôle
  ...
  private validateBusinessLogic(
    ...
    if (data.role && !this.validRoles.includes(data.role)) {
      errors.push({ 
        field: 'role', 
        message: 'Rôle invalide. Choisissez "invoice_manager" ou "dfc_agent"' 
      });
    }

    // Validation département de base
    ...
    // Validation cohérence rôle/département
  ...
  // Méthodes utilitaires pour récupérer les données valides
